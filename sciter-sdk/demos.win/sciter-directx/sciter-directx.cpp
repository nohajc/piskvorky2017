...
// SCITER-
    ...
    hr = CompileShaderFromFile( L"sciter-directx-res/shaders.fx", "VS", "vs_4_0", &pVSBlob );
    if( FAILED( hr ) )
    {
        MessageBox( NULL,
    ...
    hr = CompileShaderFromFile( L"sciter-directx-res/shaders.fx", "PS", "ps_4_0", &pPSBlob );
    if( FAILED( hr ) )
    {
        MessageBox( NULL,
                    L"The FX file cannot be compiled.  Please run this executable from sciter-sdk/bin folder.", L"Error", MB_OK );
    ...
    // Load the Texture
    hr = D3DX11CreateShaderResourceViewFromFile( g_pd3dDevice, L"sciter-directx-res/seafloor.dds", NULL, NULL, &g_pTextureRV, NULL );
    if( FAILED( hr ) )
        return hr;
